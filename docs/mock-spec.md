...
- デモは Chrome の開発者ツールのスマホ表示で見せる。アプリ側では「スマホの枠」（縁取り・角丸・影）を表示しない（`components/layout/MobileShell.tsx`。10.16参照）
  - 画面幅が広いときは、幅430pxまで（`max-w-[430px]`）の列を画面中央に置く
  - 列の外は `docs/design-spec.md` 5.1の背景（`--brand-bg` に紫のぼかし）
  - 列の中だけがスクロールし、タブバーや下部の固定要素は列の下端に固定される
...
見た目は `docs/design-spec.md` 5.4（タイムライン）・6章（`/today`）・9章（9.4 デモ時刻、9.8 現在地・次の航路、9.9 移動の表示）に従う。
...
**本文**：白いカード「今日の航路」を上部に少し重ねて置き、その中に今日のタイムライン

- 1項目1行のリストで、縦の細い線でつながった丸印を並べる（高さは所要時間に比例させない。design-spec 5.4）
- 睡眠は1行に折りたたむ
- 現在時刻を含む予定の行を「現在地」、該当がなければ次に始まる予定の行を「次の航路」として、時刻を角丸の枠で囲んで強調する（design-spec 9.8。赤い横線は使わない）
- タスクや余白（バッファ）のブロックをタップすると、下からシート（shadcn の Sheet の `side="bottom"`。10.9参照）で詳細を表示：タイトル、時間、場所、締切、余白の候補タスク、この時間に入れた理由（reason）。シートの背景は10.21参照

**下部**：タイムラインのカードの下・タブバーの上に、紫の主ボタン（「航路を調整する」）を固定表示 → `/replan`（見た目は `docs/design-spec.md` 5.3・6章参照。10.12参照）

「この計画にする」で再計画を確定すると `/today?updated=1` に遷移する。`/today` は `updated=1` があれば「計画を更新しました」の通知をページ上部に数秒だけ1回表示し、`router.replace` で `/today` にしてクエリパラメータを消す（10.7・10.20参照）

### 2.5 `/replan` 予定変更（画面の表示名は「AIとの対話」）

見た目は `docs/design-spec.md` 4章（言葉づかい）・5.5（チャット）・5.6（候補カード）・6章（`/replan`）・9.4（デモ時刻）に従う。

**上部**：`--gradient-deep` の帯に見出し「AIとの対話」（design-spec 4・6章）。右上にデモ時刻のチップ「デモ 18:00」（`demo_now`）

画面を開いたら `GET /api/mock/clock` で時刻を読み、`demo_now` が18:00より前なら `POST /api/mock/clock` に18:00を明示して送る（10.20参照）。ヘッダーの下に、その場合だけ小さく「デモのため、時刻を18:00に進めました」と表示する（すでに18:00以降なら何も表示しない。design-spec 9.4）
...
モックで結果を返せるのは「今日は疲れた」とその言い換え（「疲れた」を含む文）だけ。それ以外は、AIの吹き出しで「このデモでは『今日は疲れた』のみ対応しています」と返し、クイックリプライをもう一度出す。

**結果**

- AIの吹き出しで要約（summary_message）
- 変更点の一覧：変更された項目だけを「変更前（取り消し線）→ 変更後」で表示し、それぞれに理由を添える（design-spec 5.6の候補カード）
- 「変更なし ◯件」は折りたたみ。開くと変更のない項目の一覧
- 「ほかの日への影響」：10/7（水）・10/8（木）に移した内容
- 「変更前と変更後を並べて見る」の切り替えで、2本のタイムラインを左右に表示（幅が狭いのでブロックは簡略表示）
- 下部に「この計画にする」と「やめておく」。結果が出たあとは入力欄を隠し、この2つのボタンだけにする

「この計画にする」→ `POST /api/plans/replan/accept` → `/today?updated=1`（10.7参照）。「やめておく」→ `/today`（何も変えない）
...
- 各マス：日付、今日は丸で囲む、締切がある日は締切の印と件数、計画がある日は薄い背景、予定の種類ごとの小さな点（最大3つ）
  - 色は `docs/design-spec.md` に従う。締切の印は締切バッジと同じ色（`--deadline-fg`、`Flag` アイコン。design-spec 5.4・9.3）で、赤は使わない。計画がある日の背景は青ではなく紫系（design-spec 2.0）。10.22参照
- マスの下に「計画あり」の週を示す細い帯
- 日付をタップ → 日表示に切り替え
...
- ブロックには文字を出さず、色で種類を表す。高さが24px以上のブロックだけ、中央に12pxのアイコン（`lib/labels.ts` の種類ごとのアイコン）を出す（10.22参照）
- グリッドの下に凡例（タスク・固定予定・食事・大切な人との時間・移動・余白・自由時間の色・アイコン・表示名）
- 列見出しかブロックをタップ → その日の日表示（ブロックの中身は日表示で見る）
- 計画のない週は固定予定だけを表示
...
- **目標**（画面の見出しは「目的地」）：現在の目標「TOEIC学習　週6時間」（`task_name` と `target_hours_per_week`）と条件（`conditions`）のチップ、「新しい目的地を相談する」ボタン（→ `/interview`）。10.23参照
- **デモ用**：デモ時刻の切り替え（07:00 ／ 18:00）、「モックをリセット」ボタン（副ボタンの見た目で、画面の一番下。10.23参照）
...
### 10.16 PC表示（スマホの枠はやめ、中央の列にする）

- デモは Chrome の開発者ツールのスマホ表示で見せるため、アプリ側の「スマホの枠」（幅390px・高さ844px・角丸・`--brand-dark` の縁取り・影）は表示しない（以前の決定を変更）
- 画面幅が広いときは、幅430pxまで（`max-w-[430px]`）の列を画面中央に置く。列の外は `docs/design-spec.md` 5.1の背景
- 本文は列の中だけでスクロールし、タブバー・入力欄・主ボタンなどの固定要素は列の下端に固定される
...
### 10.21 2.4・2.5章の整理、詳細シートの背景、`/today` の読み込み中の表示

**2.4・2.5章の古い記述をなくす**

- 2.4章の「ブロックの高さは所要時間に比例」「現在時刻の位置に赤い横線」、2.5章の見出し「予定を変更」は、`docs/design-spec.md` の5.4・6章・9章で置き換わっていたため、2.4・2.5章をその内容を参照する形に書き直した
  - タイムラインは1項目1行（高さは比例させない）。現在時刻は「現在地」「次の航路」の強調で示す（design-spec 5.4・9.8）
  - `/replan` の見出しは「AIとの対話」（design-spec 4・6章）
- あわせて、10.20の報告で確認済みの判断（対応していない入力のあとにクイックリプライをもう一度出す、結果が出たら入力欄を隠して「この計画にする」「やめておく」だけにする）も2.5章に書いた

**詳細シート（`ItemDetailSheet`）の背景**

- 背景はぼかさない。暗さは `--brand-dark` の30%にして、後ろの画面がうっすら見えるようにする
- `components/ui/sheet.tsx` の `SheetContent` に `overlayClassName` を追加し、`ItemDetailSheet` から `bg-(--brand-dark)/30` とぼかしなしを渡す（shadcn の既定の背景は変えない）

**`/today` の読み込み中の表示**

- 読み込み中は、ヘッダーの挨拶と日付の位置にスケルトンを出す。見出し「今日の航路」はカードの見出しと重なるため、ヘッダーには出さない

### 10.22 `/calendar` の月表示の色、週表示のブロック

**月表示の色（2.6章の食い違いの解消）**

- 2.6章の「締切がある日は赤い点」「計画がある日は薄い青の背景」は、`docs/design-spec.md` 2.0（青・緑を主役にしない、赤はエラーだけ）・5.4・9.3（締切バッジは赤を使わない）と食い違っていたため、design-spec を参照する形に直した
- 締切の印は締切バッジと同じ色（`--deadline-fg`）の `Flag` アイコンと件数。計画がある日の背景は紫系（`--kind-task-bg`）、「計画あり」の帯も紫系

**週表示のブロック**

- 列の幅が狭く（約44px）文字が読めないため、ブロックには文字を出さない。色（design-spec 2.3 の薄い背景＋丸印の色の細い線。移動・余白は破線の枠）で種類を表す
- 高さが24px以上のブロックだけ、中央に12pxのアイコンを出す。アイコンは `lib/labels.ts` の種類ごとのアイコン（design-spec 5.4・9.3・9.7。移動は移動手段で決める）
- グリッドの下に凡例を置く：タスク・固定予定・食事・大切な人との時間・移動・余白・自由時間の色・アイコン・表示名（睡眠は凡例に入れない）
- ブロックの中身（タイトル・時刻）は日表示で見る。列見出しかブロックを押すと、その日の日表示に切り替える。タップ領域を44px以上にするため、押せる範囲はその日の列全体とする
...
- データ（`GoalSchema`）に「730点」の項目はないため、推測で組み立てない。`task_name` と `target_hours_per_week` から「TOEIC学習　週6時間」と表示し、「現在600点、目標730点」などは `conditions` のチップで表示する。期限（`deadline`）があれば「期限 12月13日（日）」も出す
- 見出しは「目的地」（design-spec.md 4章）
- 2.7章の例「TOEIC 730点　週6時間」をこの表示に合わせて修正済み

**ボタン名**

- 「新しい目標を相談する」は、design-spec.md 4章の言葉づかい（目標 → 目的地）にそろえて「新しい目的地を相談する」にする（→ `/interview`）

**デモ時刻の切り替え**

- 今のデモ時刻が 07:00 でも 18:00 でもないときは、どちらも選択されていない表示のままにする

**「モックをリセット」**

- 確認ダイアログは出さない
- 押し間違えにくいように、副ボタン（design-spec.md 5.3：白の塗り、紫の文字と枠線）の見た目にし、画面の一番下（デモ用の欄の最後）に置く
