...
- 各行：左に時刻（Inter、`tabular-nums`）、中央に丸印（2.3の色）、右にタイトル（太字）と補足（小さい灰色の文字。場所や「締切 10/9」など）
...
| `/calendar` | 3枚目（あなたの航海図） | ヘッダー `--gradient-deep`「あなたの航海図」。月／週／日の切り替えはイメージの「週／月／長期」と同じ形のセグメント。下部に「今週のコンディション」風の白いカード（タスク時間・余白・自由時間の合計を細い線のグラフで） |
